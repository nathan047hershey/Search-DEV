...
export default function FavoritesPage() {
  ...
  const handleRemoveFavorite = async (githubLogin: string) => {
    try {
      const response = await fetch(`/api/favorites/${githubLogin}`, {
        method: "DELETE",
      });

      if (response.ok) {
        // Refresh the list
        const listResponse = await fetch("/api/favorites");
        if (listResponse.ok) {
            ...
            {favorites.map((favorite) => (
              ...
              >
                <div className="flex items-start gap-4">
                  <Image
                    src={favorite.avatarUrl || `https://github.com/${favorite.githubLogin}.png`}
                    alt={favorite.githubLogin}
                    width={64}
                    height={64}
                  ...
                  <div className="flex-1 min-w-0">
                    <Link
                      href={`/profile/${favorite.githubLogin}`}
                      className="block font-semibold text-gray-900 hover:text-blue-600"
                    >
                      {favorite.name || favorite.githubLogin}
                    </Link>
                    <p className="text-sm text-gray-500">@{favorite.githubLogin}</p>
                    {favorite.bio && (
                  ...
                  <button
                    onClick={() => handleRemoveFavorite(favorite.githubLogin)}

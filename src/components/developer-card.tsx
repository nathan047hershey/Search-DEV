    ...
    <div className="flex flex-col rounded-lg border border-gray-200 bg-white p-4 shadow-sm transition hover:shadow-md">
      ...
      <div className="mt-4 flex flex-wrap gap-4 text-sm text-gray-600">
        ...
        <div className="flex items-center gap-1">
          ...
          <span>{(user.public_repos || 0).toLocaleString()} repos</span>
        </div>
      </div>
      ...
      {/* Send Email Button */}
      {user.email && onSendEmail && (
        <button
          onClick={() => onSendEmail(user)}
          className="mt-3 flex items-center justify-center gap-2 rounded-lg bg-blue-600 py-2 text-sm font-medium text-white hover:bg-blue-700"
        >
